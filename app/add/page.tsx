...
function AddPageContent() {
  ...
  console.log('[AddPage] URL date param:', dateParam)
  ...
  const handleSave = async () => {
    ...
    console.log('[AddPage] saving with expense_date:', expenseDate)
    setIsSaving(true)
    try {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* Header */}
      <div className="flex items-center justify-between px-5 pt-12 pb-4 bg-[#FAF7F2]">
        ...
        <div className="flex items-center gap-1.5 bg-white border border-[#E8E0D5] rounded-full px-3 py-1.5 shadow-sm">
          <svg className="w-3.5 h-3.5 text-[#8B7355]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <rect x="3" y="4" width="18" height="18" rx="2" />
            <path strokeLinecap="round" d="M16 2v4M8 2v4M3 10h18" />
          </svg>
          <span className="text-xs font-medium text-[#8B7355]">{dateLabel}</span>
        </div>
      </div>

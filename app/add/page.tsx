...
import { CATEGORIES, CATEGORY_EMOJI, getUserId } from '@/lib/constants'
...
export default function AddPage() {
  ...
  const handleSave = async () => {
    const finalAmount = parsed ? Number(manualAmount || parsed.amount) : Number(manualAmount)
    ...
    try {
      ...
      const { data } = await res.json()
      router.push(
        `/add/success?amount=${finalAmount}&category=${encodeURIComponent(parsed.category)}&description=${encodeURIComponent(parsed.description)}&id=${data.id}`
      )
    } catch {
      setIsSaving(false)
    }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex items-center gap-3 px-5 pt-12 pb-4 bg-[#FAF7F2]">
        <Link
          href="/"
          className="w-9 h-9 flex items-center justify-center rounded-full bg-white border border-[#E8E0D5] shadow-sm"
        >
      ...
      <div className="flex-1 px-5 pb-32">
          ...
          {input && (
            <div className="flex items-center justify-between px-4 py-2 border-t border-[#E8E0D5] bg-[#FAF7F2]/50">
              <span className="text-xs text-[#8B7355]">
                {isParsing ? 'AI 解析中...' : parsed ? '解析完成' : ''}
              </span>
              <button
                onClick={() => { setInput(''); setParsed(null); setParseError('') }}
        ...
        {parseError && !isParsing && (
          <div className="bg-[#E8736C]/10 rounded-[16px] border border-[#E8736C]/20 p-4 mb-4 slide-up">
            <p className="text-sm text-[#E8736C] font-medium mb-2">{parseError}</p>
            <p className="text-xs text-[#8B7355]">請手動輸入金額並選擇分類</p>
            <div className="mt-3 flex gap-2">
              <input
                ...
                placeholder="金額"
                className="flex-1 bg-white border border-[#E8E0D5] rounded-[8px] px-3 py-2 text-sm text-[#2C2019] outline-none"
              />
              <button
                onClick={() => setShowCategoryPicker(true)}
                className="flex-1 bg-white border border-[#E8E0D5] rounded-[8px] px-3 py-2 text-sm text-[#2C2019] text-left"
              >
                {parsed?.category || '選擇分類'}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-[16px] border border-[#E8E0D5] shadow-[0_2px_12px_rgba(44,32,25,0.06)] p-5 slide-up">
            <div className="flex items-center gap-2 mb-4">
              ...
              {parsed.confidence >= 0.9 && (
                <span className="text-xs text-[#8B7355]">信心度高</span>
              )}
            </div>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[8px] text-sm font-medium border transition-all ${
                      parsed.category === cat
                        ...
                        : 'bg-[#FAF7F2] text-[#8B7355] border-[#E8E0D5]'
                    }`}
                  >

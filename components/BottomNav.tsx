...
const navItems = [
  ...
  { href: '/records', label: '記錄', icon: ListIcon },
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className="flex flex-col items-center gap-0.5 px-4 py-1 rounded-btn transition-colors"
            >
              <Icon
                className={`w-6 h-6 ${
                  isActive ? 'text-[#4CAF7D]' : 'text-[#8B7355]'
                }`}
              />
              <span
